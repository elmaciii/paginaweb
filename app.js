...
async function fetchNews() {
    ...
    try {
        // 1. Intentar cargar desde CMS (localStorage)
        const cmsNews = getCMSNews();
...
function getCMSNews() {
    try {
        ...
        console.log('📝 Datos raw del CMS:', cmsData);
        console.log('📝 Noticias parseadas del CMS:', news);
        console.log('📝 Cantidad de noticias:', news.length);
        ...
        const validNews = news.filter(article => {
            ...
            if (isValid) {
                console.log('📝 Noticia válida:', article.title);
            }
            return isValid;
        });

        console.log('📝 Noticias válidas después del filtro:', validNews.length);

        if (validNews.length > 0) {
            console.log('✅ HAY NOTICIAS DEL CMS - NO MOSTRAR PRECARGADAS');
